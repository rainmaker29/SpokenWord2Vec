...
words = list()
all_words=list()
with open("/l/users/mohammad.sayeed/scp_files_train_clean_s3prl_hubert_backup/train_clean_100_s3prl.scp", 'r') as fin1:
    for idx,line in tqdm(enumerate(fin1)):
#         if idx<200:
#             continue
#         if idx==200:
#             break
        with open(os.path.join("/l/users/mohammad.sayeed/scp_files_train_clean_s3prl_hubert_backup/", line.split("\n")[0]), 'rb') as fin2:
            data = pickle.load(fin2)
            for feat in data:
                all_words.append(feat[1])
                if not isinstance(feat[0],str):
...
model_save_dir = "./audioEncoder_20_epochs_s3prl_devclean_hubert_backup.pt"
...
# df.to_csv("hubert_sgns.csv")
def test_word_2_cluster(x):
    return s_model.wv.index_to_key[s_model_wv_index_to_key.index(x)]

ntest_words=list()
for w in test_words:
    if w in s_model_wv_index_to_key and w in t_model.wv.index_to_key and len(s_model.wv.index_to_key)>s_model_wv_index_to_key.index(w):
        ntest_words.append(w)
print(len(ntest_words),len(set(ntest_words)))
...
for w in seed_words:
    if w in s_model_wv_index_to_key and w in t_model.wv.index_to_key and len(s_model.wv.index_to_key)>s_model_wv_index_to_key.index(w):
        nseed_words.append(w)
print(len(seed_words),len(nseed_words))
seed_words = nseed_words
del nseed_words
