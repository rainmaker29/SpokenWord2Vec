...
for fidx,set_name in enumerate(['dev-clean']):#,'train-clean-100',
    ...
    for speaker_id in tqdm(os.listdir(set_dir)):
        ...
        with open("/l/users/mohammad.sayeed/scp_files_train_clean_s3prl_wav2vec2/"+str(speaker_id)+".scp","wb") as f:
            pickle.dump(feats,f)
...
test_clean_scp = os.listdir("/l/users/mohammad.sayeed/scp_files_dev_clean_s3prl_wav2vec")
# create a Path object with the path to the file
with open("/l/users/mohammad.sayeed/scp_files_dev_clean_s3prl_wav2vec/train_clean_100_s3prl.scp","w") as f:
    for t in test_clean_scp:
        if Path('/l/users/mohammad.sayeed/scp_files_dev_clean_s3prl_wav2vec/'+t).is_file():
            if "t" in t:
                continue
            f.write(t)
            f.write("\n")